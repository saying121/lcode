...
impl LeetCode {
    ...
    pub async fn sync_problem_index(&self) -> Result<()> {
        futures::stream::iter(CATEGORIES)
            .for_each_concurrent(None, |category| async move {
                let all_pb_url = glob_config().mod_all_pb_api(category);
    ...
    pub async fn new_sync_index(&self) -> Result<()> {
        let url = &glob_config().urls.graphql;
    ...
    pub async fn submit_code(&self, idslug: IdSlug) -> Result<(SubmitInfo, RunResult)> {
        ...
        json.insert("lang", glob_config().config.lang.clone());
        ...
        let resp_json = fetch(
            &self.client,
            &glob_config().mod_submit(&pb.question_title_slug),
            Some(json),
            SendMode::Post,
            self.headers.clone(),
        )
    ...
    pub async fn test_code(&self, idslug: IdSlug) -> Result<(TestInfo, RunResult)> {
        ...
        let resp_json = match fetch(
            &self.client,
            &glob_config().mod_test(&pb.question_title_slug),
            Some(json),
            SendMode::Post,
            self.headers.clone(),
        )
    ...
    async fn get_test_res(&self, test_info: &TestInfo) -> Result<RunResult> {
        ...
        loop {
            ...
            let resp_json = fetch(
                &self.client.clone(),
                &glob_config().mod_submissions(&test_info.interpret_id),
                None,
                SendMode::Get,
                self.headers.clone(),
            )
