...
pub fn draw_info(f: &mut Frame, app: &mut App, area: Rect) {
    let chunks = Layout::default()
        ...
        .constraints([
            Constraint::Min(11),
            Constraint::Max(app.info.keymaps_items.len() as u16 + 3),
        ])
        .split(area);
    assert!(chunks.len() >= 2);
    ...
    let keymap_list = List::new(app.info.keymaps_items.clone())
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title_alignment(Alignment::Center)
    ...
    f.render_stateful_widget(keymap_list, chunks[1], &mut app.info.keymaps_state);
}
